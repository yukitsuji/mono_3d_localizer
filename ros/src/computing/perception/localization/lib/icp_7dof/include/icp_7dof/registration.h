...
public:
	...
	void align(typename pcl::PointCloud<pcl::PointXYZ> &output, const Eigen::Matrix<float, 4, 4> &guess);
	...
	void setInputSource(typename pcl::PointCloud<pcl::PointXYZ>::Ptr input);
	...
	void updateInputTarget(typename pcl::PointCloud<pcl::PointXYZ>::Ptr new_cloud);
...
protected:
	...
	typename pcl::PointCloud<pcl::PointXYZ>::Ptr source_cloud_;

	//Transformed point clouds
	typename pcl::PointCloud<pcl::PointXYZ> trans_cloud_;
	...
	// Reference map point
	typename pcl::PointCloud<pcl::PointXYZ>::Ptr target_cloud_;
};
}
